...
** Real Time Clock
Для збереження часу використовується модуль RTC DS3231, налаштовано
відповідно [[https://pimylifeup.com/raspberry-pi-rtc/][цього мануала]].
*** Синхронізація з системної дати з RTC під час старту системи
Для синхронізації використовується oneshot-сервіс /etc/systemd/system/hwclock-sync.service.
#+BEGIN_SRC ini
[Unit]
Description=Sync system clock from Hardware Clock (RTC)
